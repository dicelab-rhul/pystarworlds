...
class GridEnvironment(Environment):
   ...
   def notifyVisualPerception(self):   
        for ag in self.getAmbient().getAgents(): 
          per1=VWGridVisionPerception(ag.getID(),self.getAmbient().getPlacementMap())
          sensors = self.getPhysics().EventSensorsDirectory[type(per1)] # get sensors to be notified subscribed to that event type
          for s in sensors:
